...
>
> **Companion document:** wallets, the payment/escrow/payout mechanism, the full quality-grading system, and the complete filter set for all four portals are specified in detail in **Part 2 — `pola-payments-wallet-quality-filters-spec.md`**. This file (Part 1) stays the system-wide overview; Part 2 is the deep-dive on those specific subsystems.
...
> Every item below now has a fully specified mechanism — data states, workflows, and configuration — in **Part 2** (`pola-payments-wallet-quality-filters-spec.md`). What follows here is the short version; Part 2 is the one to build from.
